...
BTot = 512;           % Total feedback bits = 1280
...
% Load file dữ liệu H-UMi_NLOS.mat và lưu giá trị vào biến
data = load('QuaDriGa/H-UMi_NLOS.mat');
H_full = data.H;

% Kiểm tra kích thước của H_full để đảm bảo là ma trận 3D
dims = size(H_full);
if length(dims) == 3
    % Chia dữ liệu thành tập huấn luyện và tập kiểm tra
    H_train = H_full(:, :, 1:5000); % Tập huấn luyện: 5000 mẫu đầu tiên
    H_test = H_full(:, :, 5001:7000); % Tập kiểm tra: từ mẫu thứ 5001 đến mẫu thứ 7000
else
    error('H_full không phải là ma trận 3D.');
end
...
% Save HDL_test to a .mat file
save('HDL_test.mat', 'HDL_test');

% Save HDL_ori_reconst to a .mat file
save('HDL_ori_reconst.mat', 'HDL_ori_reconst');
