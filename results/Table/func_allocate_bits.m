...
% zUL_train_entries là dữ liệu đầu vào (presumably a 3D matrix)
% Bs là một mảng chứa các thông số, có thể chứa số cụm mong muốn
% sumd sẽ lưu trữ tổng khoảng cách từ mỗi điểm đến trung tâm cụm gần nhất
df(1) = sum(sumd)/size(zUL_train_entries,1);
% TINH TOAN giá trị trung bình của tổng khoảng cách từ mỗi điểm dữ liệu đến trung tâm cụm gần nhất, 
%và lưu kết quả vào phần tử đầu tiên của mảng df.
df_new = df;
