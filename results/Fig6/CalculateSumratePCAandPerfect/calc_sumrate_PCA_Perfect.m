...
H_reconstructed{2} = load('HDL_ori_reconst-BTot1024-Ntrain2000-CR16.mat').HDL_ori_reconst;
H_reconstructed{3} = load('HDL_ori_reconst-BTot1536-Ntrain2000-CR16.mat').HDL_ori_reconst;
H_reconstructed{4} = load('HDL_ori_reconst-BTot2048-Ntrain2000-CR16.mat').HDL_ori_reconst;
...
for j = 1:length(snr_values)
    ...
    for k = 1:nTest
        ...
        H_selected = HDL_test(:, :, selected_channels);
        ...
        sum_rates_perfect(k) = sum_rate_temp / nc;
    end
    
    % Calculate the average sum rate over all simulations
    avg_sum_rates(end, j) = mean(sum_rates_perfect); % Store in the last row
    % Display the result
    disp(['Perfect Channel - SNR = ', num2str(snr_values(j)), ' dB: ', num2str(avg_sum_rates(end, j))]);
end
