...
nTrain = 2000;          % # of training samples
nTest = 2000;           % # of test samples
CR_values = [16, 32, 64];                % Compression ratio
BTot_values = [512, 768, 1024, 1280, 1536, 1792, 2048];           % Total feedback bits
...
% Downsample H_train and H_test
fprintf('Downsampling H_train and H_test...\n')

% Downsample H_train
H_train_downsampled = zeros(na, nc, nTrain);
for i = 1:nTrain
    H_train_downsampled(:,:,i) = mean(reshape(H_train(:,:,i), [], na, nc), 1);
end

% Downsample H_test
H_test_downsampled = zeros(na, nc, nTest);
for i = 1:nTest
    H_test_downsampled(:,:,i) = mean(reshape(H_test(:,:,i), [], na, nc), 1);
end

H_train = H_train_downsampled;
H_test = H_test_downsampled;
...
for CR = CR_values
    for BTot = BTot_values
        if reduce_overhead
            fprintf('Reducing offloading overhead...\n')

            coeff = zeros(size(coeff_ori));

            if (floor(sqrt(na)) == sqrt(na))
                nX = sqrt(na);
                nY = nX;
            else
                % Find suitable values for nX and nY
                nX = 1;
                while mod(na, nX) ~= 0
                    nX = nX + 1;
                end
            ...
            for i = 1:size(coeff,2)
                % Reshape and apply 3D Fourier transform
                pcDFT = fftn(reshape(coeff_ori(:,i), nX, nY, nc));
                pcDFT = pcDFT(:);
                mask = zeros(size(pcDFT));
                [~,locs] = maxk(abs(pcDFT), na * nc / CR);
                mask(locs) = 1;
                pcDFT = pcDFT .* mask;
                % Reshape and apply inverse 3D Fourier transform
                coeff_tmp = ifftn(reshape(pcDFT, nX, nY, nc));
                coeff(:,i) = coeff_tmp(:);
            end

            coeff = func_gram_schmidt(coeff(:,1:500)); % Apply Gram-Schmidt orthogonalization
        else
            coeff = coeff_ori; % Keep original coefficients if not reducing overhead
        end
        ...
        save(['HDL_ori_reconst-BTot',num2str(BTot),'-Ntrain',num2str(nTrain),'-CR',num2str(CR),'.mat'],'HDL_ori_reconst')
    end
end
