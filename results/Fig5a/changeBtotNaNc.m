...
file_name = sprintf('H-UMi_NLOS_64_160_7000.mat');
H_full = load(file_name).H;
H_train = H_full(1:na, 1:nc, 1:nTrain);
H_test = H_full(1:na, 1:nc, nTrain+1:nTrain+nTest);

disp(size(H_train));
disp(size(H_test));

%%
% % Scale down H_train and H_test
% fprintf('Scaling down H_train and H_test...\n')
% 
% 
% Kiểm tra kích thước ma trận mới
% disp(size(H_train_scaled));
% disp(size(H_test_scaled));
% 
% 
% fprintf('Done!\n');
...
if reduce_overhead
    ...
    if (floor(sqrt(na)) == sqrt(na))
        ...
        % Find suitable values for nX and nY that are greater than 1
        nX = 2;
        while mod(na, nX) ~= 0
            nX = nX + 1;
            % Nếu vượt quá giá trị sqrt(na), thì dừng lại
            if nX > sqrt(na)
                error('Cannot find suitable values for nX and nY that are greater than 1');
            end
        end
        nY = na / nX;
    end
...
save(['rho-BTot',num2str(BTot),'Na',num2str(na),'Nc',num2str(nc),'-CR',num2str(CR),'.mat'],'rho')
